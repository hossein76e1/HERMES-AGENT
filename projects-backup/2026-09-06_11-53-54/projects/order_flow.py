...
def set_order_status(order_id: int, status: str, extra: dict | None = None) -> bool:
    ...
    if ts_field:
        ...
        vals.append(now())
    # 7-day support window starts at delivery
    if status == "delivered":
        sets.append("support_until=?")
        vals.append((datetime.now() + timedelta(days=7)).isoformat())
    if extra:
...
def support_days_left(order_id: int) -> int | None:
    """Days of 7-day support remaining; None = no support window (never delivered)."""
    conn = sqlite3.connect(ORDERS_DB)
    r = conn.execute("SELECT support_until FROM orders WHERE id=?", (order_id,)).fetchone()
    conn.close()
    if not r or not r[0]:
        return None
    try:
        ...
        return None
    left = (until - datetime.now()).days
    return max(0, left) + (1 if (until - datetime.now()).total_seconds() > 0 else 0)
...
def add_payment(order_id: int, amount: int, method: str, status: str = "pending",
    ...
    conn = sqlite3.connect(ORDERS_DB)
    cur = conn.cursor()
    # transaction_id is UNIQUE — if the customer re-sends the same receipt text, suffix it
    if transaction_id is not None:
        base, n = transaction_id, 1
        while cur.execute("SELECT 1 FROM payments WHERE transaction_id=?", (transaction_id,)).fetchone():
            n += 1
            transaction_id = f"{base} ({n})"
    cur.execute(
