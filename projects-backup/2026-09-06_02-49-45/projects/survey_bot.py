...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── 5. START PROJECT ──
        ...
        image_ok = bool(req.get("chat")) and (o.get("project_type") or "") in ("image_design", "logo_design", "graphic_design")
        set_order_status(oid, "in_progress")
        add_message(oid, "system", "پروژه شروع شد", "system")
        if image_ok:
            # image-design order → image_service generates & delivers
            await query.message.reply_text(
                f"🎨 سفارش #{oid} تصویره — داره تولید می‌شه... (تا ۲ دقیقه)",
                parse_mode="Markdown",
            )
            import image_service
            import threading
            threading.Thread(target=image_service.build_image_order, args=(oid,), daemon=True).start()
            await send_customer(context, oid, "🎨 **سفارش طراحی‌ت شروع شد!**\n\nدارم تصویرت رو می‌سازم — چند دقیقه صبر کن ⏳")
            return
        if auto_ok:
            ...
            await query.message.reply_text(
                f"🏗 ساخت خودکار سفارش #{oid} فعال شد — هوش مصنوعی خودش می‌سازه و تحویل میده.\n"
