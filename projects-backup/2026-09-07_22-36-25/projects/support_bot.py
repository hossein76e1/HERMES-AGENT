...
BASE_PRICES = {
    ...
    "dashboard": 12_000_000,
    "image_design": 1_500_000, "logo_design": 2_500_000, "graphic_design": 2_000_000,
    "custom": 20_000_000,
}
...
1. project_type — یکی از این‌ها: telegram_bot, whatsapp_bot, data_scraping, website, automation, content_generation, email_automation, price_comparison, dashboard, image_design, logo_design, graphic_design, custom
   ⚠️ اگر کاربر «طراحی عکس/تصویر/پوستر/بنر» خواست → image_design
   ⚠️ اگر «لوگو» خواست → logo_design
   ⚠️ اگر هر نوع «طراحی گرافیکی» دیگه خواست → graphic_design
   برای این سه نوع، بگو که طراحی با هوش مصنوعی انجام میشه و نتیجه چند دقیقه‌ای آماده میشه.
2. project_title — اسم کوتاه پروژه
3. description — توضیح اینکه پروژه چه کاری انجام میده (برای طراحی تصویر: دقیقاً چی باید توی تصویر باشه، سبک، رنگ‌ها)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── customer picks deliverable ──
        ...
        sup = f"\n🛡 پشتیبانی رایگان: **{days} روز باقی‌مانده**" if days else ""
        txt = "📦 **تحویل پروژه**\n\n"
        if files:
            txt += f"📎 فایل‌ها: {files}\n"
        if url:
            txt += f"🔗 لینک: {url}\n"
        txt += sup + ("\n\nسوالی دربارهٔ پروژه یا تغییر جزئی خواستی، دکمه 🔧 رو بزن! 🎉" if days else "\n\n⛔️ دورهٔ پشتیبانی ۷ روزه تموم شده — برای تغییرات جدید سفارش جدید ثبت کن.")
        kb = []
        if url:
            kb.append([InlineKeyboardButton("🔗 باز کردن لینک", url=url)])
        if days:
            kb.append([InlineKeyboardButton("🔧 درخواست تغییر / پشتیبانی", callback_data=f"supp:{oid}")])
        await query.message.reply_text(txt, parse_mode="Markdown",
                                       reply_markup=InlineKeyboardMarkup(kb) if kb else None)

    # ── post-delivery support / change request ──
        ...
        days = flow.support_days_left(oid)
        if not days:
            # expired → offer a new order (fresh support window after next payment)
            await query.message.reply_text(
                "⛔️ **دورهٔ ۷ روزهٔ پشتیبانی این سفارش تموم شده.**\n\n"
                "برای تغییرات جدید، سفارش جدید ثبت کن — بعد از پرداخت، ۷ روز پشتیبانی دوباره فعال می‌شه 🔄",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛒 ثبت سفارش جدید", callback_data="new_order")]]),
            )
            return
        context.user_data["mode"] = "support_change"
        context.user_data["support_order_id"] = oid
        await query.message.reply_text(
            f"🛡 **پشتیبانی — {days} روز باقی‌مانده**\n\n"
            "تغییر یا سوالی که داری رو بنویس:\n\n"
            "✅ **رایگان:** راهنمایی اجرا، رفع باگ، تغییرات جزئی\n"
            "💰 **با پرداخت جدید:** تغییرات کلی و اساسی (مثل افزودن ماژول جدید) — اول بررسی می‌کنیم و قیمت میدیم\n\n"
            "پیامت رو بنویس 👇",
            parse_mode="Markdown",
        )
...
def _user_label(user) -> str:
    """Safe markdown label for a Telegram user — escapes username underscores."""
    name = ('@' + user.username) if user.username else (user.first_name or 'کاربر')
    return md_escape(name)

# ══════════════════════════════════════════════════════════════════════════════
# Message router: receipt / talk / AI order / free support
# ══════════════════════════════════════════════════════════════════════════════
def _active_support_orders(chat_id: int) -> list[tuple[int, int]]:
    """Delivered orders of this chat with an active 7-day support window → [(order_id, days_left)]."""
    import sqlite3 as _s3x
    try:
        conn = _s3x.connect(flow.ORDERS_DB, timeout=10)
        conn.row_factory = _s3x.Row
        rows = conn.execute(
            "SELECT id FROM orders "
            "WHERE status='delivered' AND json_extract(requirements, '$.chat.chat_id') IN (?, ?) "
            "ORDER BY id DESC",
            (str(chat_id), int(chat_id) if str(chat_id).isdigit() else -1),
        ).fetchall()
        conn.close()
    except Exception:
        return []
    out = []
    for r in rows:
        days = flow.support_days_left(r["id"])
        if days and days > 0:
            out.append((r["id"], days))
    return out
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── 0. post-delivery support routing ──
    # If this chat has a delivered order with an active 7-day support window,
    # route support/help/edit requests to the support flow — NOT to new-order intake.
    if mode not in ("awaiting_receipt", "talk_to_admin", "support_change", "free_support", "ai_order"):
        active = _active_support_orders(chat_id)
        if active:
            oid, days = active[0]
            context.user_data["mode"] = "support_change"
            context.user_data["support_order_id"] = oid
            await update.message.reply_text(
                f"🛡 **پشتیبانی سفارش فعال** ({days} روز باقی‌مانده)\n\n"
                "بنویس چه تغییری می‌خوای یا سوالت چیه 👇\n\n"
                "• تغییر جزئی و راهنمایی → رایگان\n"
                "• تغییر اساسی → قیمت جدید اعلام می‌شه\n\n"
                "_اگه سفارش جدید می‌خوای، بنویس «سفارش جدید»_",
                parse_mode="Markdown",
            )
            return

    # ── 0b. explicit new-order intent escapes support mode ──
    if mode == "support_change" and ("سفارش جدید" in text or "سفارش جدید" in text.lower()):
        context.user_data.clear()
        context.user_data["mode"] = "ai_order"
    ...
    if mode == "awaiting_receipt":
        ...
        caption = (f"🧾 **رسید پرداخت جدید!**\n\n{order_card(o)}\n\n"
                   f"👤 کاربر: {_user_label(user)} ({user.id})\n"
    ...
    # ── 2b. post-delivery support / change request ──
    if mode == "support_change":
        ...
        if not o:
            await update.message.reply_text("❌ سفارش پیدا نشد — دوباره تلاش کن.")
            return
        days = flow.support_days_left(o["id"])
        if not days:
            await update.message.reply_text("⛔️ دورهٔ پشتیبانی تموم شده. برای تغییرات، سفارش جدید ثبت کن 🛒")
            return
        # log in thread + notify admin with triage keyboard
        add_message(o["id"], "customer", text, "support")
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ انجامش میدم (رایگان)", callback_data=f"adm:sfree:{o['id']}:{user.id}")],
            [InlineKeyboardButton("💰 نیاز به پرداخت جدید", callback_data=f"adm:squote:{o['id']}:{user.id}")],
            [InlineKeyboardButton("💬 پاسخ متنی", callback_data=f"adm:sreply:{o['id']}:{user.id}")],
        ])
        await notify_admin(context,
                           f"🛡 **درخواست پشتیبانی مشتری** ({days} روز باقی‌مانده)\n"
                           f"📦 سفارش `{o['order_number']}`\n"
                           f"👤 {_user_label(user)} ({user.id})\n"
                           f"━━━━━━━━━━━\n{md_escape(text)}\n━━━━━━━━━━━",
                           kb=kb)
        await update.message.reply_text(
            "✅ درخواستت رسید! بررسی می‌کنیم:\n"
            "• تغییر جزئی باشه → رایگان انجامش میدیم\n"
            "• تغییر اساسی باشه → قیمت جدید بهت اعلام می‌شه",
        )
        return
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Photo while awaiting receipt = receipt image (process directly — Message is immutable)."""
    mode = context.user_data.get("mode")
    if mode != "awaiting_receipt":
    ...
    user = update.effective_user
    photo_file_id = update.message.photo[-1].file_id
    caption_txt = (update.message.caption or "").strip()

    method = o.get("payment_method") or "card"
    add_payment(oid, o["final_price"], method, "pending",
                transaction_id=f"photo-{oid}-{int(datetime.now().timestamp())}",
                gateway_response="customer-submitted-photo")
    set_order_status(oid, "payment_review")
    add_message(oid, "customer", "[عکس رسید ارسال شد]" + (f" + یادداشت: {caption_txt}" if caption_txt else ""), "receipt_photo")

    caption = (f"🧾 **رسید عکس پرداخت!**\n\n{order_card(o)}\n\n"
               f"👤 کاربر: {_user_label(user)} ({user.id})\n"
               + (f"📝 یادداشت مشتری: {md_escape(caption_txt)}" if caption_txt else "🔗 عکس رسید ضمیمه است"))
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ پرداخت تأیید شد", callback_data=f"adm:payok:{oid}"),
        InlineKeyboardButton("❌ رسید نامعتبر", callback_data=f"adm:paybad:{oid}"),
    ]])
    try:
        ...
        logger.warning(f"photo download failed: {e}")
        queue_admin_message(oid, caption + "\n⚠️ (دانلود عکس شکست خورد — از مشتری بخواه دوباره بفرستد)", kb)
    await update.message.reply_text("✅ عکس رسیدت ثبت شد! تا چند دقیقه بررسی می‌کنیم و نتیجه رو همین‌جا می‌گم 🙏")
    context.user_data.clear()
        ...
        try:
            if image_path and os.path.exists(image_path):
                with open(image_path, "rb") as img:
                    await context.bot.send_photo(chat_id, photo=img, caption=text, parse_mode="Markdown", reply_markup=kb)
