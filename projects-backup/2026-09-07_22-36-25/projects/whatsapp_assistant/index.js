...
async function handleAction(action, jid, originalText) {
  switch (action.action) {
    ...
    case 'reminder_add': {
      const rem = { id: Date.now(), jid, text: action.text || originalText, when: action.when || null, repeat: action.repeat || 'none' };
    ...
    default: {
      ...
      return action.reply || await aiChat(ASSISTANT_SYS + '\n' + nowContext(), originalText, getHistory(jid));
    }
  }
}
...
async function sendReminder(rem) {
  ...
  if (s) await s.sendMessage(rem.jid, { text: `⏰ یادآوری:\n🔔 ${rem.text}` }).catch(() => {});
}
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    for (const m of messages) {
      ...
      try {
        ...
        const reply = await handleAction(action, jid, text);
