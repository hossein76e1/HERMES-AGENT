...
# Reminder assistant bot (@Hosseinagentcoderbot): token lives in .env (sourced above),
# not in state.db. Re-grep the file in case `source` failed.
[ -z "${TELEGRAM_ASSISTANT_TOKEN:-}" ] && TELEGRAM_ASSISTANT_TOKEN="$(grep '^TELEGRAM_ASSISTANT_TOKEN=' "$ENV_FILE" 2>/dev/null | cut -d= -f2-)"
...
check_bot "telegram_assistant/telegram_assistant.py" "TELEGRAM_ASSISTANT_TOKEN" "@Hosseinagentcoderbot"
