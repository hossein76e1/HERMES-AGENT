...
**Owner policy (Hossein, 2026-09-07):** backups live on **GitHub only**;
locally keep just the last 3 backup dirs (git history has the rest).
Disk must never exceed 80% — if cleanup can't get under it, WARN the user
instead of deleting anything essential (memory, workspace, backups).
...
**Retention policy (updated 2026-09-07 after disk hit 100%):**
- Keep last `KEEP_BACKUPS=3` backup dirs **locally** for BOTH `backups/` and
  `projects-backup/` — each run creates a full dir + tarball (30M+ each), so
  10 local copies = 270M+ and fills a small VPS disk. GitHub holds full history.
- Also prune **orphan tarballs** (tarball whose dir was already removed) —
  dir-only retention leaves ~1.7M×N of dead `.tar.gz` behind.
- The disk-monitor script (`disk_monitor.sh`) runs retention itself (cron
  every 30m, threshold 80%), so backup.sh doesn't have to; if still >80%
  after cleaning caches, it exits 2 with a Persian warning message — the
  agent must surface that warning to the owner and delete NOTHING essential
  (memories, workspace, order-system) without explicit approval.
- `git gc --prune=now` FAILS with `Out of diskspace` when the disk is already
  full (repack needs free space). Order of operations: prune old dirs first to
  free space, remove `.git/objects/*/tmp_obj_*` + `tmp_pack_*` garbage, THEN
  gc; guard gc with a free-space check (need ~2× .git size).
- Old logs (keep 50 backup logs, 30 watch logs) pruned on each run.
...
**Projects backup (separate cron job):**
- `no_agent: true`, `script: backup_projects.sh`
- Schedule: `every 12h` (staggered from main backup)
- deliver: `local`
- failure_deliver: `origin`
...
- `templates/backup.sh` — Main Hermes state backup script
- `templates/backup_projects.sh` — Project code backup script (/data/workspace)
- `templates/restore.sh` — Full VPS restore script
- `templates/support_bot.py` — Telegram support bot template
- `templates/survey_bot.py` — Telegram survey bot template