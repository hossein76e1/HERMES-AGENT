...
- Admin: @ShahbotSurveyBot (id 8681968795) — admin panel, admin_inbox worker (5s), poll_new_orders (30s), AI assistant (`/ai <request>` or plain text fallback; LLM picks a JSON action: order_info/orders_list/stats/thread/set_status/send_customer/help, executed server-side with STATUS_FA validation and outbox send).
...
## auto_builder.py — REMOVED from live flow (2026-09-07, owner decision)
All orders are now handled manually by the admin. Removal touched FOUR places:
1. `adm:start` in survey_bot.py: auto_ok/image_ok branches deleted — start now
   always sets in_progress + notifies customer; admin builds by hand.
2. The running daemon killed.
3. `bots_keeper.sh` + `restart_bots.sh`: launch blocks deleted (keeper would
   resurrect the daemon within 5 min otherwise).
4. DB: `$.auto_build` flags cleared from active (paid/in_progress) orders —
   live daemon still picks up flagged in_progress orders.
The `auto_builder.py` file itself stays on disk (owner restored it); the
historical pipeline below is kept for reference if auto-build ever returns:
- Polls status='paid' (60s); `adm:start` used to set `$.auto_build` + in_progress.
...
## image_service.py (deleted 2026-09-06; file recoverable from git history)
