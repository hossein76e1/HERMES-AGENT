...
mkdir -p "$LOG_DIR"
# Subdirectory scripts (e.g. telegram_assistant/telegram_assistant.py) log into a SUBDIR of
# LOG_DIR — create it or nohup's `>>` redirect fails and the bot never starts.
mkdir -p "$(dirname "$LOG_DIR/${SCRIPT%.py}.log")"
